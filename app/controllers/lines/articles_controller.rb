  ...
  class ArticlesController < ApplicationController
    ...
    def index
      respond_to do |format|
        format.html do
          @first_page = params[:page] && (params[:page].to_i > 0) ? false : true
          @articles = if params[:tag]
                        Article.published.tagged_with(params[:tag]).page(params[:page].to_i)
                      else
                        Article.published.page(params[:page].to_i).padding(1)
                      end
